...
- 卡密系统支持后台批次生成/CSV、用户兑换、点数/会员/组合权益、API Key、幂等外部生成、限流、停用和签名 Webhook；总开关默认关闭。
- 文本、图片、封面、返修和发布文案按实际成功的逻辑模型调用计费；模型失败或回退尝试会释放对应预留点数，自动路由候选和价格会在创建运行时冻结。
...
阶段性类型检查和相关工作流测试会随每个版本提交执行；本轮产品优化方案 A–G 的完整 lint、全量测试和构建已执行通过，真实渠道、支付和目标服务器验收仍按部署手册进行。
...
- [卡密系统设计与接口说明](./docs/06-card-code-system-design.md)
- [架构决策记录](./docs/adr/)

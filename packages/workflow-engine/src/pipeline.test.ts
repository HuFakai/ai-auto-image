...
describe("approval gate (requireApproval)", () => {
  it("holds the run at awaiting_approval until review approves", async () => {
    const harness = makeHarness({ mock: { latencyMs: 1 } });
    const runner = startEvalRunner(harness);
    const { runId, jobId } = createRunWith(harness, { topic: "审批门测试", requireApproval: true });

    await waitUntil(() => {
      const status = harness.runRepo.require(runId).status;
      return status === "awaiting_approval" || status === "failed";
    }, 20_000);
    expect(harness.runRepo.require(runId).status).toBe("awaiting_approval");
    // job 正常结束，run 挂起等待人工确认
    expect(harness.jobRepo.require(jobId).status).toBe("succeeded");

    harness.runRepo.setReview(runId, "approved");
    harness.runRepo.updateStatus(runId, "succeeded");
    await runner.stop();

    expect(harness.runRepo.require(runId).status).toBe("succeeded");
    expect(harness.assetRepo.listByRun(runId).filter((a) => a.kind === "generated")).toHaveLength(4);
  });
});

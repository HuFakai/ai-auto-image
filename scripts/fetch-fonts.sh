#!/usr/bin/env bash
# Download Noto Sans SC + Noto Serif SC (OFL license) for Satori deterministic rendering.
...
SERIF_BASES=(
  "https://raw.githubusercontent.com/googlefonts/noto-cjk/main/Serif/OTF/SimplifiedChinese"
  "https://cdn.jsdelivr.net/gh/googlefonts/noto-cjk@main/Serif/OTF/SimplifiedChinese"
)

fetch() {
  # $1 = 远端文件名（noto-cjk 仓库命名），$2 = 本地文件名（render-engine 期望），$3.. = 候选镜像
  local remote="$1"
  local file="$2"
  shift 2
...
fetch "NotoSansCJKsc-Regular.otf" "NotoSansSC-Regular.otf" "${SANS_BASES[@]}"
fetch "NotoSansCJKsc-Bold.otf" "NotoSansSC-Bold.otf" "${SANS_BASES[@]}"
fetch "NotoSerifCJKsc-Regular.otf" "NotoSerifSC-Regular.otf" "${SERIF_BASES[@]}"
fetch "NotoSerifCJKsc-Bold.otf" "NotoSerifSC-Bold.otf" "${SERIF_BASES[@]}"

...
import type { ChannelView } from "@/server/channel-service";
...
export function SettingsView({ initial }: { initial: ChannelsPayload }) {
  ...
  return (
    <div className="space-y-14 pt-10">
      <section className="rise">
        <p className="kicker">SETTINGS · 渠道管理</p>
        <h1 className="mt-3 font-display text-3xl font-black sm:text-4xl">
          模型渠道<span className="text-seal">。</span>
        </h1>
        <p className="mt-3 max-w-xl text-sm leading-relaxed text-ink-soft">
          文本与图片渠道分离配置，支持多个渠道并按顺序回退；密钥加密落库、界面只显示末四位。
          当前生效：
      ...
      >
        ...
        <div className="space-y-5">
          ...
          <p className="border-t border-line pt-3 text-[11px] leading-relaxed text-ink-faint">
            已有渠道：{channels.length} 个。保存后立即生效，无需重启；正在运行的任务继续使用启动时的渠道。
          </p>
        </div>
      </div>
    </div>

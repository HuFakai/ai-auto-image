...
# Direct script execution intentionally bootstraps the repository root before
# importing the ``api`` package. Keep this exception local to the entry point.
# ruff: noqa: E402
...
async def lifespan(app: FastAPI):
    ...
    try:
        from pixelle_video.services.video import prune_stale_video_temp_media

        temp_cleanup = prune_stale_video_temp_media()
        logger.info(
            "Temporary video cleanup complete (files={}, directories={}, bytes={}, skipped={})",
            temp_cleanup["files"],
            temp_cleanup["directories"],
            temp_cleanup["bytes"],
            len(temp_cleanup["skipped"]),
        )
    except Exception as exc:
        logger.warning("Temporary video cleanup skipped: {}", exc)
    await task_manager.start()
    runner_status = await production_runner_manager.startup()
    logger.info(
        "Continuous production runner control ready (enabled={}, state={})",
        runner_status["enabled"],
        runner_status["state"],
    )
    ...
    await production_runner_manager.stop(persist=False)
    await task_manager.stop()
    from pixelle_video.services.hyperframes_process import hyperframes_process_manager
    await hyperframes_process_manager.stop()
...
app.include_router(production_router, prefix=api_config.api_prefix)
app.include_router(projects_router, prefix=api_config.api_prefix)
app.include_router(renderers_router, prefix=api_config.api_prefix)
...
async def root():
    ...
    return {
        ...
        "api": {
            ...
            "production": f"{api_config.api_prefix}/production",

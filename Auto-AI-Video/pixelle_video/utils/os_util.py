...
def _local_ffmpeg_bin_dirs() -> list[Path]:
    """Return project-local directories that may contain ffmpeg.exe / ffprobe.exe."""
    roots: list[Path] = []
    env_root = os.environ.get("PIXELLE_VIDEO_ROOT")
    if env_root:
        roots.append(Path(env_root))
    roots.append(Path.cwd())
    # pixelle_video/utils/os_util.py -> repository root
    ...
    for root in roots:
        try:
            ...
            continue
        for candidate in (
            resolved_root / ".windows-tools" / "ffmpeg" / "bin",
            resolved_root / ".windows-tools" / "ffmpeg",
        ):
...
def ensure_local_ffmpeg_on_path() -> Optional[str]:
    """Prepend the project-local FFmpeg bin directory to PATH when present.

    The Windows one-click launcher installs FFmpeg under `.windows-tools/ffmpeg/bin`
    and only injects that directory into its own process environment. Manual
    terminal starts therefore cannot see ffmpeg via shutil.which unless we
    recover the local copy here.
    """
    existing_ffmpeg = shutil.which("ffmpeg")
    existing_ffprobe = shutil.which("ffprobe")
    if existing_ffmpeg and existing_ffprobe:
        return existing_ffmpeg

    ffmpeg_name = "ffmpeg.exe" if os.name == "nt" else "ffmpeg"
    ffprobe_name = "ffprobe.exe" if os.name == "nt" else "ffprobe"
    for bin_dir in _local_ffmpeg_bin_dirs():
...
def which_ffmpeg() -> Optional[str]:
    """Locate ffmpeg, preferring PATH then the project-local Windows portable build."""
    ensure_local_ffmpeg_on_path()
    return shutil.which("ffmpeg")


def which_ffprobe() -> Optional[str]:
    """Locate ffprobe, preferring PATH then the project-local Windows portable build."""
    ensure_local_ffmpeg_on_path()
...
def get_task_frame_path(
    ...
    frame_index: int, 
    file_type: Literal[
        "audio", "image", "video", "composed", "overlay", "subtitle", "segment"
    ]
) -> str:
    """
...
def get_resource_path(resource_type: Literal["bgm", "templates", "workflows"], *paths: str) -> str:
    ...
    Examples:
        ...
        # Returns a custom or bundled template path.
    """
...
def list_resource_dirs(
    ...
    Examples:
        ...
        # Returns the available resource subdirectories.
    """
    dirs = set()

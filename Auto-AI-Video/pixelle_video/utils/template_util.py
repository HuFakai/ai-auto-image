...
def resolve_template_fingerprint(
    template_input: Optional[str],
    expected_sha256: str | None = None,
) -> tuple[str, str]:
    """Resolve a template and verify its immutable content fingerprint."""
    candidate = Path(template_input).expanduser() if template_input else None
    resolved_path = (
        str(candidate.resolve())
        if candidate and candidate.is_absolute() and candidate.is_file()
        else resolve_template_path(template_input)
    )
    digest = hashlib.sha256(Path(resolved_path).read_bytes()).hexdigest()
...
def template_supports_layered_background(template_input: Optional[str]) -> bool:
    """Return whether a template declares a full-canvas separable media layer."""
    from bs4 import BeautifulSoup

    resolved_path, _ = resolve_template_fingerprint(template_input)
    soup = BeautifulSoup(Path(resolved_path).read_text(encoding="utf-8"), "html.parser")
    return soup.select_one('[data-pixelle-media-layer="full-canvas"]') is not None


def template_media_layer_mode(template_input: Optional[str]) -> str:
    """Return the declared media geometry contract for native composition."""
    from bs4 import BeautifulSoup

    resolved_path, _ = resolve_template_fingerprint(template_input)
    soup = BeautifulSoup(Path(resolved_path).read_text(encoding="utf-8"), "html.parser")
    layer = soup.select_one("[data-pixelle-media-layer]")
    if layer is None:
        return "none"
    value = str(layer.get("data-pixelle-media-layer") or "").strip().lower()
    return value if value in {"full-canvas", "inset"} else "unspecified"


def snapshot_template_for_task(
    template_input: Optional[str],
    task_dir: str | Path,
    expected_sha256: str | None = None,
) -> tuple[str, str]:
    """Copy an immutable template version into one task's owned directory."""
    snapshot_root = Path(task_dir) / "templates"
    if expected_sha256 and snapshot_root.is_dir():
        existing = list(snapshot_root.rglob(f"*-{expected_sha256[:12]}.html"))
        for candidate in existing:
            snapshot_digest = hashlib.sha256(candidate.read_bytes()).hexdigest()
            if snapshot_digest == expected_sha256:
                return str(candidate.resolve()), expected_sha256
    source_path, digest = resolve_template_fingerprint(template_input, expected_sha256)
    source = Path(source_path)
    size_dir = source.parent.name
    target = (
        snapshot_root
        / size_dir
        / f"{source.stem}-{digest[:12]}{source.suffix}"
    )
    ...
    if target.exists():
        ...
        if snapshot_digest != digest:
            raise RuntimeError(
                f"Task template snapshot is corrupt: expected {digest}, got {snapshot_digest}"
            )
    else:
        ...
        if snapshot_digest != digest:
            ...
            raise RuntimeError(
                "Frame template changed while its task snapshot was being created: "

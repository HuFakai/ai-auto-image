...
async def generate_narrations_from_topic(
    ...
    logger.info(f"Generating {n_scenes or 'an autonomous number of'} narrations from topic: {topic}")

    prompt = build_topic_narration_prompt(
        topic=topic, n_storyboard=n_scenes, min_words=min_words, max_words=max_words
    )

    narrations = await _request_narrations(
        llm_service,
        prompt=prompt,
        n_scenes=n_scenes,
...
async def _request_narrations(
    ...
    *,
    ...
    """Request and validate narration JSON without restarting the whole pipeline."""
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.debug(f"Narration attempt {attempt}: LLM response: {response[:200]}...")
            result = _parse_json(response)
            narrations = result.get("narrations")
            if not isinstance(narrations, list):
                raise ValueError("Invalid response format: missing 'narrations' list")
            if not narrations:
                raise ValueError("LLM returned no narrations")
            if n_scenes is None:
                return [str(item).strip() for item in narrations[:40] if str(item).strip()]
            ...
            return [str(item).strip() for item in narrations[:n_scenes]]
        except (json.JSONDecodeError, TypeError, ValueError) as exc:
            logger.warning(f"Narration JSON attempt {attempt}/{max_retries} failed: {exc}")
            if attempt >= max_retries:
                raise

    raise RuntimeError("Narration generation exhausted without a result")
    ...
    for batch_idx, batch_narrations in enumerate(batches, 1):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.error(
                    f"Batch {batch_idx} JSON parse error (attempt {attempt}/{max_retries}): {e}"
                )
                if attempt >= max_retries:
                    raise
                logger.info(f"Retrying batch {batch_idx}...")
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if progress_callback:
                    ...
                    progress_callback(
                        completed, total, f"Batch {batch_idx}/{len(batches)} completed"
                    )

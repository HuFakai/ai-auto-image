...
The TTS service uses Microsoft Edge TTS.
"""
...
_ATTEMPT_TIMEOUT = float(os.getenv("PIXELLE_EDGE_TTS_ATTEMPT_TIMEOUT", "120"))
...
async def edge_tts(
    ...
    attempt_timeout: float = _ATTEMPT_TIMEOUT,
) -> bytes:
    """
    ...
    Args:
        ...
        attempt_timeout: Total timeout for each streaming attempt (default: 120s)
    ...
    if attempt_timeout <= 0:
        raise ValueError("Edge TTS attempt_timeout must be greater than zero")
    ...
    async with request_semaphore:
        ...
        for attempt in range(retry_count + 1):  # +1 because first attempt is not a retry
            if attempt > 0:
                ...
                logger.info(
                    f"🔄 Retrying Edge TTS (attempt {attempt + 1}/{retry_count + 1}) after {retry_delay:.2f}s delay..."
                )
            ...
            try:
                ...
                # Bound the entire stream, not just individual websocket reads.
                # Audio stays in memory until complete, so a timeout cannot
                # leave an empty or partially written destination file.
                audio_chunks = []
                ...
                if not audio_data:
                    raise NoAudioReceived("Edge TTS stream completed without audio data")
                ...
                last_error = e
                logger.warning(
                    "⚠️  Edge TTS stream timed out "
                    f"after {attempt_timeout:g}s (attempt {attempt + 1}/{retry_count + 1})"
                )
                if attempt >= retry_count:
                    message = (
                        f"Edge TTS timed out after {attempt_timeout:g}s on all "
                ...
                if error_code == 401 or "401" in error_msg:
                    logger.warning(
                        f"⚠️  Edge TTS 401 Authentication Error (attempt {attempt + 1}/{retry_count + 1})"
                    )
                    logger.debug(f"Error details: {error_msg}")
                    logger.debug(
                        "This is usually caused by rate limiting. Will retry with exponential backoff..."
                    )
                else:
                    logger.warning(
                        f"⚠️  Edge TTS error (attempt {attempt + 1}/{retry_count + 1}): {error_code} - {e}"
                    )

                if attempt >= retry_count:
                    # Last attempt failed
                    logger.error(
                        f"❌ All {retry_count + 1} attempts failed. Last error: {error_code}"
                    )
                    raise
                ...
                logger.debug(
                    "This is usually a temporary Microsoft service issue. Will retry with longer delay..."
                )
...
async def list_voices(
    locale: str = None, retry_count: int = _RETRY_COUNT, retry_base_delay: float = _RETRY_BASE_DELAY
) -> list[str]:
    """
    ...
    async with request_semaphore:
        ...
        for attempt in range(retry_count + 1):
            if attempt > 0:
                ...
                logger.info(
                    f"🔄 Retrying list voices (attempt {attempt + 1}/{retry_count + 1}) after {retry_delay:.2f}s delay..."
                )
                await asyncio.sleep(retry_delay)
            ...
            try:
                ...
                logger.info(
                    f"Found {len(voice_ids)} voices" + (f" for locale '{locale}'" if locale else "")
                )
                return voice_ids
                ...
                if error_code == 401 or "401" in error_msg:
                    logger.warning(
                        f"⚠️  Edge TTS 401 Authentication Error (list_voices attempt {attempt + 1}/{retry_count + 1})"
                    )
                    logger.debug(f"Error details: {error_msg}")
                    logger.debug(
                        "This is usually caused by rate limiting. Will retry with exponential backoff..."
                    ...
                    logger.warning(
                        f"⚠️  List voices error (attempt {attempt + 1}/{retry_count + 1}): {error_code} - {e}"
                    )

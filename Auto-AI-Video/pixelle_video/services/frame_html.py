...
class TrustedHTML(str):
    """Markup produced by Pixelle's own escaping/highlight pipeline."""
...
class HTMLFrameGenerator:
    ...
    def __init__(self, template_path: str, expected_sha256: str | None = None):
        """
        ...
        self.template_sha256 = hashlib.sha256(Path(template_path).read_bytes()).hexdigest()
        if expected_sha256 and self.template_sha256 != expected_sha256:
            raise RuntimeError(
                "Frame template content changed after the task was created: "
                f"expected {expected_sha256}, got {self.template_sha256}"
            )
    ...
    def has_safe_layer(self, kind: str) -> bool:
        """Return whether the template declares a separately renderable safe layer."""

        quoted = re.escape(kind)
        return bool(
            re.search(
                rf"data-pixelle-safe\s*=\s*(['\"]){quoted}\1",
                self.template,
                flags=re.IGNORECASE,
            )
        )
    ...
    @staticmethod
    def _layer_visibility_css(layer_mode: str) -> str:
        """Build deterministic visibility isolation for split native overlays."""

        if layer_mode == "full":
            return ""
        subtitle = '[data-pixelle-safe="subtitle"]'
        if layer_mode == "chrome":
            return f"""
            {subtitle}, {subtitle} * {{
              visibility: hidden !important;
            }}
            """
        if layer_mode == "subtitle":
            return f"""
            ...
            """
        raise ValueError(f"Unsupported frame layer_mode: {layer_mode}")
    ...
    def _replace_parameters(self, html: str, values: Dict[str, Any]) -> str:
        ...
        def replacer(match):
            ...
            if param_name in values:
                ...
                return html_lib.escape(default_value_str, quote=True)
    ...
    async def _ensure_browser(cls):
        ...
        if not browser_usable:
            ...
            try:
                cls._browser = await cls._playwright.chromium.launch(args=launch_args)
            except Exception as exc:
                message = str(exc).lower()
                if "executable doesn't exist" not in message and "playwright install" not in message:
                    raise
                logger.warning(
                    "Playwright-managed Chromium is missing; falling back to the system Chrome channel"
                )
    ...
    async def close_browser(cls):
        ...
        if cls._browser:
            try:
                ...
                logger.debug(f"Ignoring error while closing Playwright browser: {exc}")
            finally:
                cls._browser = None
        if cls._playwright:
            try:
                ...
                logger.debug(f"Ignoring error while stopping Playwright: {exc}")
            finally:
                cls._playwright = None
    ...
    async def generate_frame(
        ...
        Args:
            ...
            transparent_background: Remove the declared media/background layer
            layer_mode: Render all content, static chrome without subtitle, or subtitle only
        ...
        try:
            ...
            try:
                ...
                focus_x = float(context.get("focus_x", 50))
                focus_y = float(context.get("focus_y", 50))
                await page.add_style_tag(
                    content=f"""
                    [data-pixelle-media-layer] {{
                      background-position: {focus_x:.3f}% {focus_y:.3f}% !important;
                    }}
                    [data-pixelle-media-layer] > img,
                    [data-pixelle-media-layer] > video,
                    img[data-pixelle-media-layer],
                    video[data-pixelle-media-layer] {{
                      object-position: {focus_x:.3f}% {focus_y:.3f}% !important;
                    }}
                    """
                )
                if transparent_background:
                    await page.add_style_tag(
                        content="""
                        html, body, [data-pixelle-media-layer] {
                          background: transparent !important;
                          background-color: transparent !important;
                          background-image: none !important;
                        }
                        [data-pixelle-media-layer][src],
                        [data-pixelle-media-layer] > img,
                        [data-pixelle-media-layer] > video {
                          visibility: hidden !important;
                        }
                ...
                layout = await self._measure_safe_layout(page)
                layer_css = self._layer_visibility_css(layer_mode)
                if layer_css:
                    await page.add_style_tag(content=layer_css)
                await page.screenshot(path=output_path, type='png', omit_background=True)
                layout_path = Path(output_path).with_suffix('.layout.json')
    ...
    async def _measure_safe_layout(self, page) -> Dict[str, Any]:
        """Measure marked text after shrinking it into a vertical-video safe area."""
        safe_area = {
            'left': round(self.width * 0.075),
            'top': round(self.height * 0.05),
            'right': round(self.width * 0.925),
            'bottom': round(self.height * 0.80),
        }
        elements = await page.evaluate(
            """
            (safe) => Array.from(document.querySelectorAll('[data-pixelle-safe]')).map((element) => {
              const bounds = () => {
                const rect = element.getBoundingClientRect();
                return {left: rect.left, top: rect.top, right: rect.right, bottom: rect.bottom,
                  width: rect.width, height: rect.height};
              };
              const within = (rect) => rect.left >= safe.left && rect.top >= safe.top &&
                rect.right <= safe.right && rect.bottom <= safe.bottom;
              const original = bounds();
              const computed = window.getComputedStyle(element);
              let fontSize = Number.parseFloat(computed.fontSize) || 16;
              const minimum = Number(element.dataset.pixelleSafeMinFont || 28);
              let current = original;
              let attempts = 0;
              if (element.dataset.pixelleSafeAutoFit !== 'false') {
                while (!within(current) && fontSize > minimum && attempts < 40) {
              ...
              return {
                kind: element.dataset.pixelleSafe,
                bounds: current,
                original_bounds: original,
                adjusted: attempts > 0,
                final_font_size: fontSize,
                in_safe_area: within(current),
              };
            })
            """,
            safe_area,

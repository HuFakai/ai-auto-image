...
| 批次 | 内容 | 依赖 | 预估 |
|---|---|---|---|
| 1 | PostgreSQL 切换 ✅（PG 方言 + PGlite 测试 + 远程建库与数据导入） | — | 已完成 |
| 2 | 用户登录注册 ✅（scrypt + DB 会话 + 邀请码注册 + 资源隔离 + 微信扫码预留） | — | 已完成 |
| 3 | 服务器部署物料 ✅（compose/ARM/Dockerfile/docs/deployment.md；服务器上执行构建与反代按手册操作） | — | 物料完成 |
| 4 | 内容类型扩展（金句卡/清单卡/对比卡/产品种草/图书推荐/长文拆解/四格漫画） | 上线前后皆可 | 每类 0.5–1.5 天 |
| 5 | Brand Kit / 品牌手册增强 ✅（水印/签名/字体/色板/封面模板/零费用实时预览） | — | 已完成 |

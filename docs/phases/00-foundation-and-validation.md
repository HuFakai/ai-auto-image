...
- 用户请求并发、服务器最大并发和 Provider 最大并发分开保存。
- 默认请求并发为 1，服务器最大值通过环境变量设置，初期建议不超过 4。
- 验证并发 1、2、4 时的内存、下载和 SQLite 写入表现。
- 图片 API 调用和 Sharp 后处理使用不同信号量，Sharp 默认并发 1。
- 验证 429、超时、取消和动态降并发。
...
- 图片生成默认并发为 1，用户可以在服务器上限内调整；任何设置都不能突破管理员安全上限。
- Sharp 后处理默认并发为 1，连续生成时不会因无限解码大图触发 OOM。

...
> 本文是当前实现的状态基线。后续开发优先以本文和 [当前开发路线图](./current-roadmap.md) 为准；早期规划、阶段设计和 ADR 用you于保留背景，不再自动代表当前待办。

## 一、结论

项目已经从“工程初始化”进入**可演示 Beta / 上线前加固期**。当前已经形成从主题或资料输入、内容规划、图片生成、文字渲染、逐页返修、品牌套用、评审到 ZIP 导出的主要闭环，并补齐了登录、资源归属、套餐点数、支付订单和管理后台的第一版。
...
- 六套确定性渲染主题、中文字体、Logo、水印、签名、色板和封面模板配置已接入 Brand Kit。
...
`wallets.reserved_credits` 和 `workflow_runs.credits_reserved/credits_charged` 已加入迁移。知识卡片、漫画、封面候选、单页返修和局部重绘均在图片 Provider 调用前原子预留；成功图片从预留中结算，失败/取消释放剩余额度，重试按已结算数量补足。deterministic 非 default 版式是纯排版，不调用图片 Provider，也不扣图像点数；deterministic rerender 同样不收费。

...
| 文字确定性渲染 | 已删除 | 新建、重试和单页重生统一走模型原生中文图片 |
...
- 构建阶段不再下载字体，也不依赖 `packages/render-engine/assets/fonts` 目录；这避免了全新 clone 构建时因字体目录缺失失败。
...
- 新建和重试任务统一使用图片模型直接生成中文最终图片；项目已删除 deterministic 文字确定性渲染入口。中文、价格、规格或 CTA 仍应通过预期文案、质量检查和失败重试复核。
...
| Docker 构建提示字体目录不存在 | 构建日志和 Dockerfile 版本 | 旧镜像仍引用已删除的字体复制步骤；拉取最新代码后重新构建 |
| 健康正常但显示 Mock | 设置页渠道列表 | TEXT_*/IMAGE_* 未自动导入，或渠道被禁用 |
| 渠道密钥解密失败 | APP_SECRET 是否与录入时一致 | 更换了主密钥或迁移时未带旧密钥 |
| 作品生成失败 | 进入作品详情页查看失败摘要 | 使用“从检查点重试”复用已完成内容；余额充足但渠道持续失败时可“从头开始（新作品）”，也可在失败页卡片单独重试 |
| 点数明细/订单不全 | 查看页面底部分页与接口响应中的 total | 新版会自动执行 `0008_brainy_scorpion` 迁移；确认容器使用了最新代码和同一 PostgreSQL 数据库，不要只刷新旧前端缓存 |

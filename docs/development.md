...
充值中心历史记录通过 `/api/billing/ledger` 与 `/api/pay/orders` 分页读取，默认每页 20 条、服务端限制为 10–100 条；点数消费写入 Run ID 和作品标题快照。管理员订单搜索参数 `q` 同时匹配订单号与订单标题，管理员调点使用数据库事务同时写钱包、流水和 `admin_adjust` 调整订单。

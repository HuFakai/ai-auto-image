...
生产业务表位于外部 PostgreSQL，不在 `/data/db/app.db` 中。当前 PostgreSQL Schema 共 31 张表，其中包含卡密系统的批次、单卡、兑换、API Key、幂等、Webhook 和审计表。`/data` 仍必须使用持久卷并备份，因为资产、导出文件和可能的密钥文件都在其中。
...
充值中心历史记录通过 `/api/billing/ledger` 与 `/api/pay/orders` 分页读取，默认每页 20 条、服务端限制为 10–100 条；点数消费写入 Run ID 和作品标题快照。管理员订单搜索参数 `q` 同时匹配订单号与订单标题，管理员调点使用数据库事务同时写钱包、流水和 `admin_adjust` 调整订单。卡密兑换使用独立的 `card_redeem` 订单/流水类型，不计入支付收入。
...
- storage、Web 的 PGlite/计费集成测试已配置合理默认超时；全仓测试 19/19 个任务通过，卡密并发兑换专项测试通过。
- 支付 mock 生产硬关闭、额度预留/结算/释放、重复节点结算保护和当前 22 张旧 SQLite 迁移表工具已纳入代码与测试；卡密新增迁移为 `0010_card_code_system.sql`。
